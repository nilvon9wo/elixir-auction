defmodule AuctionTest do
  ...
  describe "insert_item/1" do
    ...
    test "the Item in the database has teh attributes provided" do
      attributes = %{title: "test item", description: "test description"}
      {:ok, item} = Auction.insert_item(attributes)
      assert item.title == attributes.title
      assert item.description == attributes.description
    end
